...
def get_k(match_type: str) -> float:
    if match_type=="q":
        return pipeline_config.k_quarter
    elif match_type=="w":
        return pipeline_config.k_worldcup
    elif match_type=="f":
        return pipeline_config.k_friendly
    return pipeline_config.k_tournament

def expected(r_a: float, r_b: float) -> float:
    return 1/(1+10**((r_b - r_a)/400))

def gd_mult(goal_diff: int) -> float:
    return max(1, np.log1p(min(abs(goal_diff), pipeline_config.gd_cap)) / np.log(2))
...
def add_elo(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in df.itertuples(index=False):
        ...
        r_home_eff = r_home + (0 if row.neutral else pipeline_config.home_advantage)
